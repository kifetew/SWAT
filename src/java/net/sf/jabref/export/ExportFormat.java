...
public class ExportFormat implements IExportFormat {
	...
	public void performExport(final BibtexDatabase database,
		...
		// Set a global field, so all layouts have access to the custom name
		// formatters:
		...
		if (missingFormatters.size() > 0) {
			StringBuilder sb = new StringBuilder(
					"The following formatters could not be found").append(": ");
			...
			try {
				...
				if (in != null) {
					...
					for (int i = 0; i < lines.length; i++) {
						...
						if (line.length() == 0)
							continue;
						int index = line.indexOf(":"); // TODO: any need to
														// accept escaped colons
														// here?
